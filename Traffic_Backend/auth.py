...
# In-memory fallback removed — using DB-backed users.
...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(lambda: next(get_db()))):
    ...
    if user_obj is None:
        raise credentials_exception
    roles = user_obj.roles.split(',') if user_obj.roles else []
    return {"username": user_obj.username, "roles": roles}
...
def create_user(db: Session, username: str, password: str, email: str = None, roles: str = ''):
    hashed = pwd_context.hash(password)
    user = User(username=username, email=email, hashed_password=hashed, roles=roles)
    db.add(user)
    db.commit()
    db.refresh(user)
    return user
